...
checkpoint_dir = 'MCAN_checkpoints/ADFT/rep'
...
for epoch in range(num_epochs):
    ...
    for episode in tqdm(range(episodes), desc=f'Epoch {epoch + 1}/{num_epochs}', leave=True):
        ...
        s, q, pids = episodic_sampling(3, 5, train_classes, dataset, n_query=1)
        ...
        for step in range(steps):
            ...
            #     ytest, cls_scores = task_model(s_f.view(1, 12, 1, 128, 93).to(device), q_f.to(device), slabel_f, qlabel_oh)
            ...
            #     ytest, cls_scores = task_model(s_f.view(1, 3 * (j + 1), 1, 128, 93).to(device), q_f.to(device), slabel_f, qlabel_oh)
            #     # loss1 = criterion(ytest, pids.view(-1))
            #     loss = criterion(cls_scores, qlabel.view(-1))
            #     # loss = loss1 + 0.5 * loss2
            #     task_model.adapt(loss, allow_unused=True)
    ...
    if (epoch + 1) % 2 == 0:
        ...
        for episode in tqdm(range(val_eps), leave=True):
            ...
            s_v, q_v, pids_v = episodic_sampling(3, 5, test_classes, dataset, n_query=1)
            ...
            for step in range(steps):
                ...
                #     q_f = torch.stack([s_v[i, j + 1] for i in range(s_v.size(0))]).unsqueeze(0)
