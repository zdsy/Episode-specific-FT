...
from utils import wav_episodic_sampling, audio_to_melspectrogram
import torch
import numpy as np
import os
...
if batch == 1:
    ...
    episodes = 100
steps = 8
checkpoint_dir = 'MC_checkpoints/5-way-5-shot/ADFT/pitch/scale_1'
...
for epoch in range(epochs):
    ...
    for episode in tqdm(range(episodes), desc=f'Epoch {epoch + 1}/{epochs}', leave=True):
        ...
        for b in range(batch):
            ...
            with torch.no_grad():
                s_v, q_v, wav = wav_episodic_sampling(5, 5, train_classes, train_dataset)
                s_v = s_v.to(device)
    ...
    if ((epoch + 1) % 3) == 0:
        ...
        for i in tqdm(range(val_episodes), leave=True):
            ...
            with torch.no_grad():
                s_v, q_v, wav = wav_episodic_sampling(5, 5, val_classes, val_dataset)
                s_v = s_v.to(device)
                q_v = q_v.unsqueeze(1).to(device)
